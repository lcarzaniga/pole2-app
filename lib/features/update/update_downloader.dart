...
class UpdateDownloader {
  ...
  Stream<DownloadState> run(UpdateRelease release) async* {
    ...
    try {
      ...
      _client = http.Client();
      final resp = await _client!.send(http.Request('GET', Uri.parse(release.apkUrl)));
      ...
      try {
        await for (final chunk in resp.stream) {
          ...
          if (total > 0) {
            yield DownloadState(DownloadStage.downloading,
    ...
    // 3. Launch the system installer for the verified file.
    try {
