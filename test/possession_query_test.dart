...
void main() {
  ...
  test('search matches title or category, case-insensitively', () async {
    ...
    final out = applyPossessionQuery(
        [bike, lamp], const PossessionQuery(search: 'BIKE'));
  ...
  test('sort by name orders alphabetically, case-insensitively', () async {
    ...
    final a = await make('Alpha');
    final out = applyPossessionQuery(
        [b, a], const PossessionQuery(sort: PossessionSort.name));
  ...
  test('filter by a specific place, no place, or all', () async {
    ...
    expect(
      applyPossessionQuery(all, PossessionQuery(place: PlaceFilter.place(garage)))
          .map((p) => p.title),
      ['Trapano'],
    );
    expect(
      applyPossessionQuery(all, const PossessionQuery(place: PlaceFilter.none()))
          .map((p) => p.title),
      ['Chiavi'],
    );
    expect(
      applyPossessionQuery(all, const PossessionQuery()).length,
      2,
    );
  });
}
